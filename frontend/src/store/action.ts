...
import { adaptClientToCreateRent, adaptClientToUpdatedRent, adaptCommentsToClient, adaptCommentToClient, adaptFullRentToClient, adaptRentsToClient, adaptRentToClient } from '../utils/adapter';
...
export const fetchOffers = createAsyncThunk<Offer[], undefined, { extra: Extra }>(
  ...
  async (_, { extra }) => {
    const { api } = extra;
    const { data } = await api.get<RentDto[]>(ApiRoute.Rents);

    return adaptRentsToClient(data);
  });
...
export const fetchFavoriteOffers = createAsyncThunk<Offer[], undefined, { extra: Extra }>(
  ...
  async (_, { extra }) => {
    const { api } = extra;
    const { data } = await api.get<RentDto[]>(`${ApiRoute.Rents}/${ApiRoute.Favorite}`);
...
export const fetchOffer = createAsyncThunk<Offer, Offer['id'], { extra: Extra }>(
  ...
  async (id, { extra }) => {
    ...
    try {
      const { data } = await api.get<FullRentDto>(`${ApiRoute.Rents}/${id}`);

      return adaptFullRentToClient(data);
    } catch (error) {
...
export const postOffer = createAsyncThunk<Offer, NewOffer, { extra: Extra }>(
  ...
  async (newOffer, { extra }) => {
    const { api, history } = extra;
    const { data } = await api.post<FullRentDto>(ApiRoute.Rents, adaptClientToCreateRent(newOffer));
...
export const editOffer = createAsyncThunk<Offer, Offer, { extra: Extra }>(
  ...
  async (offer, { extra }) => {
    const { api, history } = extra;
    const { data } = await api.patch<FullRentDto>(`${ApiRoute.Rents}/${offer.id}`, adaptClientToUpdatedRent(offer));
    history.push(`${AppRoute.Property}/${data.id}`);
...
export const deleteOffer = createAsyncThunk<void, string, { extra: Extra }>(
  ...
  async (id, { extra }) => {
    const { api, history } = extra;
    await api.delete(`${ApiRoute.Rents}/${id}`);
...
export const fetchPremiumOffers = createAsyncThunk<Offer[], string, { extra: Extra }>(
  ...
  async (cityName, { extra }) => {
    const { api } = extra;
    const { data } = await api.get<RentDto[]>(`${ApiRoute.Rents}${ApiRoute.Premium}?city=${cityName}`);
...
export const fetchUserStatus = createAsyncThunk<UserAuth['email'], undefined, { extra: Extra }>(
  ...
  async (_, { extra }) => {
    ...
    try {
      const { data } = await api.get<LoginDto>(ApiRoute.Auth);
...
export const registerUser = createAsyncThunk<void, UserRegister, { extra: Extra }>(
  ...
  async ({ email, password, name, avatar, type }, { extra }) => {
    const { api, history } = extra;
    const { data } = await api.post<{ id: string, token: string }>(ApiRoute.Users, {
      email,
      password,
      name,
      isPro: type === 'pro',
    ...
    if (avatar) {
      ...
      await api.post(`${ApiRoute.Users}${ApiRoute.Avatar}`, payload, {
...
>(Action.POST_FAVORITE, async (id, { extra }) => {
  ...
  try {
    const { data } = await api.patch<RentDto>(
      `${ApiRoute.Rents}/${id}/favorite`,
    );

    return adaptRentToClient(data);
  } catch (error) {
    const axiosError = error as AxiosError;
